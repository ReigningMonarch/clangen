...
class Pregnancy_Events():
    ...
    def handle_two_moon_pregnant(cat: Cat, clan=game.clan):
        ...
        for kit in kits:
            if random.randint(1,4) == 1:
                ...
                game.cur_events_list.append(Single_Event(choice(possible_woods_str).replace("k_n", str(kit.name)), "birth_death"))
                ...
                if random.randint(1,20) == 1 and not parent_reaction:
                    ...
                    try:
                        ...
                        if not parent.dead:
                            ...
                            game.cur_events_list.append(Single_Event(f"{parent.name} screams their fury into the sky. Why, oh why did the woods have to take their kit from them? Claws unsheathed, they swear their revenge and sprint into the woods, never to be seen again.", "birth_death"))
                    except:
                        continue
    # ---------------------------------------------------------------------------- #
    ...
    def get_second_parent(cat, clan):
        ...
        affair_partner = Pregnancy_Events.determine_love_affair(cat, mate, mate_relation, samesex)
        if affair_partner:
